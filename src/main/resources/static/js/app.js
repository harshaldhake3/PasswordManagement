...
(function(){
  ...
  const strengthOf = (p)=>{
    ...
    return ["Weak","Okay","Good","Strong","Excellent"][s];
  };
  const initGenerator = ()=>{
    const gen = document.getElementById('gen-pass');
    const pass = document.querySelector('input[name="password"]');
    const badge = document.getElementById('strength');
    if (!pass) return;
    const upd = ()=>{
      if (badge && pass.value) badge.textContent = "Strength: " + strengthOf(pass.value);
    };
    if (gen) gen.addEventListener('click', ()=>{
      pass.value = generatePassword();
      upd();
    });
    if (pass) pass.addEventListener('input', upd);
    upd();
  };
  const initCopy = ()=>{
    const copyU = document.getElementById('copy-user');
    const copyP = document.getElementById('copy-pass');
    const get = (sel)=>document.querySelector(sel)?.textContent;
    if (copyU) copyU.addEventListener('click', ()=>{
      const u = document.querySelector('dd[th\:text="${cred.loginUsername}"]')?.textContent || document.querySelector('dd:nth-of-type(2)')?.innerText;
      navigator.clipboard.writeText(u||'');
      toast('Username copied');
    });
    if (copyP) copyP.addEventListener('click', ()=>{
      const p = document.getElementById('passwordValue')?.innerText || '';
